...
The layer ID is sanitized to only alphanumeric characters and underscores, everything else is replaced with underscores. This keeps the instrument name valid for OTEL.  This can cause collisions, usage for "my-layer" and "my_layer" will get written the same metrics.
