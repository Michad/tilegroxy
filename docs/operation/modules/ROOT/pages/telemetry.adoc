...
Metrics are aggregates and are constrained in cardinality, which makes them suited to answering questions about the health of the service. If instead you need to know which layers a given user or tenant consumed, see xref:configuration/analytics/index.adoc[Analytics] which records individual events attributable to a layer and a user.
