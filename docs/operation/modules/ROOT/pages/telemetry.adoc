...
| The number of xref:configuration/analytics/index.adoc[analytics] events successfully written to a destination
...
The layer ID is sanitized to only alphanumeric characters, underscores, and hyphens; everything else is replaced with underscores.  It is also truncated to 200 characters to leave room for the metric name around it.  This keeps the instrument name valid for OTEL.  Either step can cause collisions, so usage for "my.layer" and "my_layer" will get written to the same metrics.

The `tilegroxy.analytics.*` metrics are meant to help monitor the health of xref:configuration/analytics/index.adoc[Analytics] delivery.  A spike in `dropped` or `error` count means usage tracking is getting dropped, so it's worth alerting on these separately.
