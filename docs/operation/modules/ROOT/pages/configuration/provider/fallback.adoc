...
The fallback provider is not meant to be used to handle auth errors with custom providers. The preAuth method is never called for the secondary provider and, if the primary provider returns an auth error, the secondary provider will not be called (the re-auth behaviour for the primary provider will occur as normal). Any auth error handling should be built into your custom provider itself.
