...
This can only be used with layers that return JPEG or PNG images. The result is always output in PNG format at the same size as the tile from the downstream provider.
