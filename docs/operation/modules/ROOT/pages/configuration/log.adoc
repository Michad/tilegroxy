...
* *config_reload*: The configuration was reloaded, which only happens when xref:reloading.adoc[hot reloading] is enabled. The `outcome` attribute is either `success` or `failure`, and a failure carries the `error` that stopped it.
