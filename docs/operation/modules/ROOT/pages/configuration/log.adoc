...
The `Logging` section supports three keys: `main`, `access` and `audit` which are objects described respectively below.
...
Configures a log containing only security relevant events. Auditing is off by default.

Two kinds of event are emitted, each identified by an `event` attribute:

* *auth_failure*: A request was rejected for an authentication or authorization reason. This covers a failed check by the configured xref:configuration/authentication/index.adoc[authentication], a request for a layer the caller isn't allowed, and a request for a tile outside the caller's allowed geographic area. The `reason` attribute names which of these occurred. Standard request fields are always included (URI, path, protocol, IP, method, host, and user agent) and, when possible, `user` and `tenant` are included as well.
* *config_reload*: The configuration was reloaded, which happens when xref:reloading.adoc[hot reloading] is enabled or when a watched xref:configuration/secret/index.adoc[secret] rotates. The `outcome` attribute is either `success` or `failure`, and a failure carries the `error` that stopped it. The `reason` attribute says what triggered the reload: `config_file` for a change to the configuration file, `secret_rotation` for a watched secret whose value changed, and `secret_ttl` for the periodic reload a secret `ttl` schedules.
...
| Enabled
| Whether to emit audit events at all
| bool
| No
| false

| Console
| Whether to write audit events to standard out
| bool
| No
| true

| Path
| The file location to write audit events to. Log rotation is not built-in, use an external tool to avoid excessive growth. Required if Console is false.
| string
| No
| None

| Format
| The format to output audit events in. Applies to both standard out and file out. Possible values: plain, json
| string
| No
| json

| Headers
| Headers to include as attributes on audit events. Attribute key will be in all lowercase.
| string[]
| No
| None
|===
...
| Console
| LOGGING_AUDIT_CONSOLE

| Path
| LOGGING_AUDIT_PATH

| Format
| LOGGING_AUDIT_FORMAT

| Headers
| LOGGING_AUDIT_HEADERS
|===
