...
See xref:content-type.adoc[Content Type] for details on how this is handled.
...
Every cache configured at the top level is checked, not just the default one, so a cache used only by a layer that xref:configuration/layer.adoc[overrides its cache] is still monitored. Use `caches` to narrow that down. The check fails if any of the caches it covers fails, and the output names which ones.
...
| caches
| The ids of the caches to check. Every id must match a cache declared in xref:configuration/cache/index.adoc[cache]
| string[]
| No
| Every configured cache
