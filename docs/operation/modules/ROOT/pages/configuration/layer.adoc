...
| cachettl
| Seconds after which a cached tile for this layer is treated as stale and regenerated. Enforced uniformly regardless of which xref:configuration/cache/index.adoc[cache] is configured, emulating it for a backend with no native expiry. Has no effect if skipcache is true
| int
| No
| Cache backend's own default/native behavior applies uninterfered with
