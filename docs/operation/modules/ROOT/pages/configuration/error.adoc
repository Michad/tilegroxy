...
Error responses set a `Content-Type` matching the mode: `text/plain; charset=utf-8` for Text mode, `image/png` for Image and Image with Header modes. None mode sets no body and thus no `Content-Type`.

Every error response also sets `Cache-Control: no-store`, regardless of mode. This matters most in combination with `AlwaysOk` below: with `AlwaysOk` set, an error is returned with HTTP status 200 just like a real tile, so without an explicit `no-store` a CDN or browser sitting in front of tilegroxy could cache a "tile unavailable" response as if it were a legitimate tile - and keep serving that cached error long after the underlying provider issue is resolved. `Cache-Control: no-store` prevents that regardless of `AlwaysOk`.
