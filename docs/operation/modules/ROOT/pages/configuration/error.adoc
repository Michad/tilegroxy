...
Error responses set a `Content-Type` matching the mode: `text/plain; charset=utf-8` for Text and `image/png` for Image and Image with Header.  None mode returns no body and therefore no `Content-Type`.
