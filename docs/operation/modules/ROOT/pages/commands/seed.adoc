...
Flags:
  ...
  -l, --layer string            The ID of the layer to seed
      --max-failures uint       How many tiles can fail to render before
                                the seed gives up and exits with a failure
                                status.
                                Defaults to the number of tiles being
                                seeded, meaning the seed only fails if
                                every tile does
  ...
  -p, --progress string         A file to use to record how far the seed 
      ...
      --purge                   Delete the cached tiles covering the
                                ...
                                populating them.
      --tenant string           The tenant ID to seed as, standing in for
                                what authentication would supply on a real
                                request. Required to seed a specific
                                tenant's tiles when using a tenant cache
  -t, --threads uint16          How many concurrent requests to use to
      ...
      --user string             The user ID to seed as, standing in for
                                what authentication would supply on a real
                                request
