...
A caller with valid credentials who's restricted from a specific layer or area receives the same 404 Not Found response as a request for a layer that was never configured, rather than a 401. This prevents an authenticated-but-restricted caller from probing layer names and inferring which ones exist in your configuration just from whether they get "forbidden" or "not found". Only a request with missing, invalid, or expired credentials receives a 401.