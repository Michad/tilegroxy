...
An Operator can configure a layer whose tiles depend on who requested them, by interpolating the identity into the upstream request or by cropping to an area the requester's credentials allow. The cache key carries no identity, so such a layer leaks tiles between users unless it uses a xref:configuration/cache/tenant.adoc[tenant] cache or skips caching. See xref:howto/user-dynamic.adoc[User Dynamic Layers].
...
* Content Type validation - if the server returns an invalid content-type then the request errors. By default only PNG, JPEG, and vector tile payloads are allowed. See xref:content-type.adoc[Content Type] for the full default allowlist.
...
Tilegroxy has a few separately configurable log streams available. Consider ingesting each of these into your SIEM or log aggregator if your system is security-sensitive. 

Most notably from a security side, rejected requests and configuration reloads can be recorded to a dedicated xref:configuration/log.adoc[audit log]. 