...
There are two ways to extend tilegroxy. One is to use the various "custom" options to provide interpreted Go code to implement a provider, authentication scheme, or analytics destination. The other is to use tilegroxy as a library and create your own executable with whatever tweaks you need.
...
Your custom code must live within a single file for each provider/auth/analytics.  It can use the entire standard library including potentially dangerous function calls such as `exec` and `unsafe`; be as cautious using custom providers from third parties as you would be executing any other third party software.

There is a performance cost of using a custom vs a built-in offering. The exact cost depends on the complexity of your code, however it is typically below 10 milliseconds while tile generation as a whole is usually more than an order of magnitude slower. With authentication especially tilegroxy utilizes caching to mitigate this impact.  However it is something you should keep in mind when deciding whether to implement a Custom provider/auth. Due to these being written in Go, it is easy to convert your custom code to a built-in equivalent if you find this overhead becomes a bottleneck.
...
Example custom providers can be found within link:/examples/providers/custom_proxy.go[examples/providers/custom_proxy.go].

Custom providers must be within the `custom` package and must import the `tilegroxy/tilegroxy` package for mandatory datatypes. 
...
And one optional function:

[,go]
----
func close(tilegroxy.Context) error
----
...
The `close` function is for releasing any resources your provider holds, such as a database handle or a background goroutine. It will be called either when tilegroxy shuts down or during a hot reload.
...
You can provide a custom xref:configuration/analytics/index.adoc[analytics] for recording usage events to wherever. The package must be "custom" and you must include the following function:

----
func record(tilegroxy.Context, []tilegroxy.AnalyticsEvent, map[string]interface{}, tilegroxy.ErrorMessages) error
----
...
An `AnalyticsEvent` contains `Time`, `LayerID`, `LayerName`, `Z`, `X`, `Y`, `UserID` and a `Fields` map holding the attributes selected via `fields` and `extraFields`.

The `record` method is called with a batch of events, not once per request. Returning an error causes the batch to be logged as failed and discarded. 

When present, `close` is called when tilegroxy shuts down or when analytics is torn down as part of a hot reload. It runs after the final batch has been flushed.
...
// Declares what kind of tile this provider produces. Return config.DataTypeRaster or config.DataTypeMVT if it's always one or the other, or config.DataTypeUnknown if it depends
func (s SampleRegistration) DataType(cfgAny any) config.DataType {
...
// Everything the provider is given at construction arrives in the deps struct: deps.ClientConfig, deps.ErrorMessages, deps.LayerGroup and deps.Datastores.
func (s SampleRegistration) Initialize(cfgAny any, deps layer.ProviderDeps) (layer.Provider, error) {
...
A provider that can describe its own tiles can also implement the optional `layer.MetadataProvider` interface. Its `Metadata()` method returns a `config.LayerMetadata`, the same struct the layer configuration embeds, whose data type, zoom range, bounds, and TileJSON fields act as defaults for the layer. Anything set in the layer configuration still wins. It can be called more than once during startup, so keep it cheap.
