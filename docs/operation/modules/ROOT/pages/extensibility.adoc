...
This is for releasing any resources your provider holds, such as a database handle or a background goroutine. When present, it's called with a context carrying the shutdown deadline, both when tilegroxy shuts down and when the layer is torn down as part of a config hot reload.
...
This is for releasing any resources your authentication scheme holds. When present, it's called with a context carrying the shutdown deadline, both when tilegroxy shuts down and when authentication is torn down as part of a config hot reload.

=== Custom Analytics

A custom xref:configuration/analytics/index.adoc[analytics] module records usage events to whatever destination you need.  Example custom analytics modules can be found within link:../examples/analytics/[examples/analytics].  The package must be "custom" and you must include the following function:

----
func record(tilegroxy.Context, []tilegroxy.AnalyticsEvent, map[string]interface{}, tilegroxy.ErrorMessages) error
----

The arguments are the context, the batch of events, the parameters included under the analytics module in the configuration, and a mapping for localization of error messages.  An `AnalyticsEvent` contains `Time`, `LayerID`, `LayerName`, `Z`, `X`, `Y`, `UserID` and a `Fields` map holding the attributes selected via `fields` and `extraFields`.

Unlike providers and authentication, `record` is called once per *batch* of events rather than once per request.  This keeps the interpretation overhead described above off the per-tile path and means the batching parameters control how often your code runs.

Returning an error causes the batch to be logged as failed and discarded. It never affects the tile responses that produced those events.
...
This is for releasing any resources your analytics module holds beyond the batching tilegroxy already manages for you. When present, it's called with a context carrying the shutdown deadline, both when tilegroxy shuts down and when analytics is torn down as part of a config hot reload. It runs after the final batch has been flushed.
