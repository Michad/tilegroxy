...
// Declares what kind of tile this provider produces. Return pkg.DataTypeRaster or pkg.DataTypeMVT if it's always one or the other, or pkg.DataTypeUnknown if it depends
func (s SampleRegistration) DataType(cfgAny any) pkg.DataType {
