...
* [ ] If using xref:configuration/analytics/index.adoc[analytics], size `batch.queueSize` for your peak request rate and alert on the `tilegroxy.analytics.dropped` and `tilegroxy.analytics.error` metrics so data loss doesn't go unnoticed
* [ ] If using analytics, ensure your destination has a retention or partitioning strategy since tilegroxy never expires events, and review whether the fields you collect (particularly `ip` and `useragent`) are compatible with your privacy policy
