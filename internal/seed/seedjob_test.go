...
func collect(e *SeedJob, start uint64) ([]uint64, []pkg.TileRequest) {
	var indexes []uint64
...
func Test_SeedJob_Zoom0(t *testing.T) {
	e, err := NewSeedJob("test", world(), []uint{0})
...
func Test_SeedJob_OrderIsDeterministic(t *testing.T) {
	...
	shuffled, err := NewSeedJob("test", world(), []uint{2, 1, 0, 1})
	require.NoError(t, err)
...
func Test_SeedJob_RestrictiveBounds(t *testing.T) {
	e, err := NewSeedJob("test", pkg.Bounds{South: 51, North: 51.6, West: 5.7, East: 7.0, SRID: pkg.SRIDWGS84}, []uint{8})
	require.NoError(t, err)
...
func Test_SeedJob_FromAtEndYieldsNothing(t *testing.T) {
	e, err := NewSeedJob("test", world(), []uint{0, 1})
	require.NoError(t, err)
...
func Test_SeedJob_StopsEarlyWhenCallerBreaks(t *testing.T) {
	e, err := NewSeedJob("test", world(), []uint{0, 1, 2})
	require.NoError(t, err)
...
func Test_SeedJob_InvalidZoom(t *testing.T) {
