...
func Test_Progress_RoundTrip(t *testing.T) {
	e, err := NewEnumeration("osm", world(), []uint{0, 1, 2})
...
func Test_Progress_MatchesDetectsChangedRun(t *testing.T) {
	...
	differentZoom, err := NewEnumeration("osm", world(), []uint{0, 1, 3})
	require.NoError(t, err)
	assert.False(t, p.Matches("osm", differentZoom))

	extraZoom, err := NewEnumeration("osm", world(), []uint{0, 1, 2, 3})
	require.NoError(t, err)
	assert.False(t, p.Matches("osm", extraZoom))

	differentBounds, err := NewEnumeration("osm", pkg.Bounds{South: 10, North: 20, West: 10, East: 20, SRID: pkg.SRIDWGS84}, []uint{0, 1, 2})
	require.NoError(t, err)
...
func Test_Progress_SaveOverwritesCleanly(t *testing.T) {
	...
	e, err := NewEnumeration("osm", world(), []uint{1})
	require.NoError(t, err)
...
func Test_DefaultProgressPath(t *testing.T) {
	assert.Equal(t, filepath.Join("etc", "tilegroxy-seed-osm.progress.json"), DefaultProgressPath(filepath.Join("etc", "tilegroxy.yml"), "osm"))

	// The layer name is operator supplied but ends up in a filename, so it can't carry separators.
	assert.Equal(t, "tilegroxy-seed-______etc_passwd.progress.json", DefaultProgressPath("", "../../etc/passwd"))
}
