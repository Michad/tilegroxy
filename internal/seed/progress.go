...
// DefaultProgressPath is where progress is recorded when no location is given: next to the
// configuration file, named for the layer being seeded. Layer names are operator supplied but still
// end up in a filename, so anything that isn't a plain name is replaced.
func DefaultProgressPath(configPath, layerName string) string {
	safe := make([]rune, 0, len(layerName))

	for _, r := range layerName {
	...
	name := fmt.Sprintf("tilegroxy-seed-%v.progress.json", string(safe))
...
func NewProgress(layerName string, e *Enumeration) *Progress {
	return &Progress{
...
func LoadProgress(path string) (*Progress, error) {
	raw, err := os.ReadFile(path) // #nosec G304 -- the path is operator supplied, same as the config path it defaults beside
	if err != nil {
...
func (p *Progress) Matches(layerName string, e *Enumeration) bool {
