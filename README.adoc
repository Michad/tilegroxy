...
Everything outside of `config` covers the usual deployment concerns: replicas, resources, ingress, autoscaling, storage and scheduling.  Credentials should not be placed in `config` since it becomes a ConfigMap; instead reference them as `env.VAR_NAME` in the configuration and supply the value with the chart's `secrets` or `existingSecretKeys` values.
