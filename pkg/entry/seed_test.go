...
func Test_Seed_ForceAllowsExcessiveTileCount(t *testing.T) {
	e, err := seed.NewEnumeration("counts", pkg.WorldBounds(), []uint{10})
	require.NoError(t, err)
...
// The estimate only exists to give the tile count a sense of scale, so it just has to scale sensibly
// with the number of threads.
func Test_EstimateDuration(t *testing.T) {
	assert.InDelta(t, 2*estimateDuration(100000, 8).Seconds(), estimateDuration(100000, 4).Seconds(), 60)
	assert.Greater(t, estimateDuration(1000000, 1), estimateDuration(100000, 1))
}
...
func Test_Seed_ResumesFromRecordedPosition(t *testing.T) {
	...
	e, err := seed.NewEnumeration("counts", pkg.WorldBounds(), []uint{0, 1, 2})
	require.NoError(t, err)
...
func Test_Seed_ResumeFromCompletedRun(t *testing.T) {
	...
	e, err := seed.NewEnumeration("counts", pkg.WorldBounds(), []uint{0, 1})
	require.NoError(t, err)
...
func Test_Seed_RefusesMismatchedProgressFile(t *testing.T) {
	...
	e, err := seed.NewEnumeration("counts", pkg.Bounds{South: 10, North: 20, West: 10, East: 20, SRID: pkg.SRIDWGS84}, []uint{0, 1, 2})
	require.NoError(t, err)
...
// Rerunning without --resume starts over, but says so rather than quietly discarding the progress.
func Test_Seed_WithoutResumeWarnsAboutExistingProgress(t *testing.T) {
	cfg := seedTestConfig(t)
	path := filepath.Join(t.TempDir(), "progress.json")
	...
	_, rendered := seedTestCounter.snapshot()
	assert.Len(t, rendered, int(e.Count()))
}
...
func Test_DefaultProgressPath(t *testing.T) {
	assert.Contains(t, DefaultProgressPath(filepath.Join("etc", "tilegroxy.yml"), "osm"), "tilegroxy-seed-osm")
}
